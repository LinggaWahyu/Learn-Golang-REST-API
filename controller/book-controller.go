...
import (
	"Learn-Golang-API/dto"
	"Learn-Golang-API/entity"
	"Learn-Golang-API/helper"
...
func (c *bookController) FindByID(context *gin.Context) {
	...
	if (book == entity.Book{}) {
		res := helper.BuildErrorResponse("Data was not found", "No data with given id", helper.EmptyObj{})
...
func (c *bookController) Delete(context *gin.Context) {
	...
	id, err := strconv.ParseUint(context.Param("id"), 0, 0)
	if err != nil {
		res := helper.BuildErrorResponse("No param id was found", "No param id were found", helper.EmptyObj{})
		context.AbortWithStatusJSON(http.StatusBadRequest, res)
	}
